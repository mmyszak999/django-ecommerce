...
from src.apps.orders.entities.cart_entities import CartItemEntity, CartItemUpdateEntity
...
class CartItemCreateService:
    def _cart_item_create(
        self, dto: CartItemEntity, product: Product, cart: Cart
    ) -> CartItem:
        return CartItem.objects.create(
            product=product, cart=cart, quantity=dto.quantity
        )

    @classmethod
    def _build_cart_item_dto_from_request_data(
...
class CartItemUpdateService:
    def _cart_item_update(
        self, dto: CartItemUpdateEntity, instance: CartItem
    ) -> CartItem:
    ...
    @classmethod
    def _build_cart_item_dto_from_request_data(
        cls, request_data: OrderedDict, instance: CartItem
    ) -> CartItemUpdateEntity:
        serializer = CartItemQuantityUpdateSerializer(data=request_data)
        serializer.is_valid(raise_exception=True)
        data = dict(serializer.validated_data)

        return CartItemEntity(data.get("quantity", instance.quantity))
